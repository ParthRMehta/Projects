...
class GUI extends JFrame implements ActionListener
{
  ...
  JTextField oFNField = new JTextField(System.getProperty("user.dir")+"\\report.xlsx");
  ...
  public JPanel createPanel2()
  {
    ...
    public void keyReleased(KeyEvent e) {
        try
        {
           final URL url = new URL(sampleLinkField.getText());
	   ...
	   else if(!url.getFile().endsWith("Marksheet.html")) status.setText("Something seems wrong..(Check the tutorial)");
           else status.setText("");
        }
	catch(Exception ex)
        {
   ...
   JScrollPane scroll = new JScrollPane(uidField, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
	...
	public void actionPerformed(ActionEvent e)
	{
	  ...
	  jFileChooser.setCurrentDirectory(new File((new File(oFNField.getText())).getParent()));	  
	...
	try
	{
	    ...
	    public void onMarksheetEntryFailed(String UID)
	    {
	      ...
	      cancelButton.setVisible(false);
            ...
            {
	      jProgressBar.setValue(jProgressBar.getValue()+1);
	      invalidUIDModel.addRow(new String[]{UID});
              return true;
            }
          }); 
